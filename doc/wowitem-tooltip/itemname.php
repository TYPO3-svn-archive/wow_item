...
if(!file_exists($filename) && is_numeric($_GET['id']))
{
	...
	{
		$link = '<a href="' . ARMORY . '/item-info.xml?i='.$arr['itemtooltips']['itemtooltip']['id'].'" target="new" onmouseover="tooltip_itemid(\''.$arr['itemtooltips']['itemtooltip']['id'].'\',\''.$_GET['lang'].'\')" onmouseout="tooltip_close()" style="color:'.color($arr['itemtooltips']['itemtooltip']['overallqualityid']).'">['.$arr['itemtooltips']['itemtooltip']['name'].']</a>';
	...
	if(is_array($arr[0]))
	{
		foreach($arr as $item)
		{
			if(strtolower(trim($item['name'])) == strtolower(trim($_GET['id'])))
			{
				...
				{
					$link[] = '<a href="' . ARMORY . '/item-info.xml?i='.$item_x['itemtooltips']['itemtooltip']['id'].'" target="new" onmouseover="tooltip_itemid(\''.$item_x['itemtooltips']['itemtooltip']['id'].'\',\''.$_GET['lang'].'\')" onmouseout="tooltip_close()" style="color:'.color($item_x['itemtooltips']['itemtooltip']['overallqualityid']).'">['.$item_x['itemtooltips']['itemtooltip']['name'].']</a>';
